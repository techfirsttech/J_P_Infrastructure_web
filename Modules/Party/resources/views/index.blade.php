@extends('layouts.app')
@section('title', __('party::message.party'))
@section('content')
<div class="row">
     <div class="col-12 mb-2">
          <h5 class="content-header-title float-start mb-0">{{ __('party::message.list') }}</h5>
     ...
     <div class="modal-dialog modal-dialog-centered modal-sm">
          <div class="modal-content">
               <div class="modal-header bg-transparent">
                    <h5 class="text-center mb-0" id="exampleModalTitle">{{ __('party::message.add') }}</h5>
                         ...
                         <div class="row">
                              <div class="col-12 col-sm-12 col-md-12 col-lg-12 form-group custom-input-group">
                                   <label class="form-label"
                                        for="party_name">{{ __('party::message.party_name') }} <span
                                             class="text-danger">*</span></label>
                                   <input type="hidden" name="id" id="id" value="">
                                   <input type="text" class="form-control" name="party_name" id="party_name"
                                        placeholder="{{ __('party::message.party_name') }}">
                                   <span class="invalid-feedback d-block" id="error_party_name" role="alert"></span>
                              </div>
                              ...
                              <div class="col-12 col-sm-12 col-md-12 col-lg-12 mt-1">
                                   ...
                                   <button type="submit" class="btn btn-sm btn-primary float-end save"
                                        data-route="{{ route('party.store') }}">{{ __('message.common.submit') }}</button>
                              </div>
                         </div>
                    </form>
               </div>
...
<script type="application/javascript">
     ...
     $("#inlineModal").on("hidden.bs.modal", function(e) {
          ...
          $("#exampleModalTitle").html("{{ __('party::message.add') }}");
     });

     $(document).on('click', '.edit', function() {
          $('.modal').modal('hide');
          var id = $(this).data('id');
          var url = "{{route('party.edit','id')}}".replace('id', id);
          $.ajax({
               type: "GET",
               url: url,
               dataType: 'json',
               ...
               success: function(response) {
                    if (response.status_code == 200) {
                         $("#exampleModalTitle").html("{{ __('party::message.edit') }}");
                         $("#party_name").val(response.result.party_name);
                         $("#id").val(id);
     ...
     $("#form").validate({
          ...
          messages: {
               party_name: {
                    required: "{{ __('party::message.enter_party_name') }}",
               }
          },
          errorElement: "p",
