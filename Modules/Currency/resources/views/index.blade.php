...
@section('content')
    <div class="row">
        <div class="col-12 mb-2">
            <h5 class="content-header-title float-start mb-0">{{ __('currency::message.list') }}</h5>
            @can('currency-create')
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-header bg-transparent">
                    <h5 class="text-center mb-0" id="exampleModalTitle">{{ __('currency::message.add') }}</h5>
                    ...
                    <form id="form" action="javascript:void(0);" method="POST">
                        ...
                        <div class="row">
                            <div class="col-12 col-sm-12 col-md-12 col-lg-12 form-group custom-input-group">
                                <label class="form-label" for="currency_name">{{ __('currency::message.name') }} <span
                                        class="text-danger">*</span></label>
                                <input type="hidden" name="id" id="id" value="">
                                <input type="text" class="form-control" name="currency_name" id="currency_name"
                                    placeholder="{{ __('currency::message.name') }}">
                                <span class="invalid-feedback d-block" id="error_currency_name" role="alert"></span>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-12 form-group custom-input-group">
                                <label class="form-label" for="currency_symbol">{{ __('currency::message.symbol') }} <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="currency_symbol" id="currency_symbol"
                                    placeholder="{{ __('currency::message.enter_symbol') }}">
                                <span class="invalid-feedback d-block" id="error_currency_symbol" role="alert"></span>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-12 mt-1">
                                ...
                                <button type="submit" class="btn btn-sm btn-primary float-end save"
                                    data-route="{{ route('currency.store') }}">{{ __('message.common.submit') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
...
@section('pagescript')
    <script type="application/javascript">
     'use strict';
     const URL = "{{route('currency.index')}}";
     var table = '';
     $(function() {
     ...
     $("#form").validate({
          ...
          messages: {
               currency_name: {
                    required: "{{ __('currency::message.enter_name') }}",
               },
                currency_symbol: {
                    required: "{{ __('currency::message.enter_symbol') }}",
               },
          },
          errorElement: "p",
