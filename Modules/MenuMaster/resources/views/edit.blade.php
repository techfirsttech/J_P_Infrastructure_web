...
<div class="row">
    <div class="col-12">
        <h4 class="content-header-title float-start">{{ __('menumaster::message.menu_item_edit') }}</h4>
        <div class="float-end">
            ...
            <a href="{{ route('menumasters.show', $menuMaster) }}" class="btn btn-sm btn-info">
                <i class="fas fa-eye"></i> {{ __('menumaster::message.view_details') }}
            </a>
        </div>
    </div>
    <div class="col-12">
        <div class="card p-1">
            <div class="card-body">
                ...
                <div class="alert alert-info">
                    <h6><i class="fas fa-info-circle"></i> {{ __('menumaster::message.current_position') }}:</h6>
                    <strong>{{ __('menumaster::message.order') }}:</strong> {{ $menuMaster->getHumanReadableOrder() }} |
                    <strong>{{ __('menumaster::message.level') }}:</strong> {{ $menuMaster->getLevel() }} |
                    <strong>{{ __('menumaster::message.parent') }}:</strong>
                    {{ $menuMaster->parent ? $menuMaster->parent->menu_title : __('menumaster::message.none_main_menu') }}
                </div>

                <form action="javascript:void(0);" id="menuForm" method="POST" autocomplete="none">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-12 col-md-6 col-lg-3 form-group custom-input-group">
                            ...
                            <input type="text" class="form-control" name="if_can" id="if_can"
                        ...
                        {{-- Main Menu Checkbox --}}
                        <div class="col-12 col-sm-12 col-md-6 col-lg-3 form-group custom-input-group">
                            <label class="form-label">{{ __('menumaster::message.menu_options') }}</label>
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input" id="is_main_menu" name="is_main_menu" value="1"
                                    {{ old('is_main_menu', $menuMaster->is_main_menu) ? 'checked' : '' }}>
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 form-group custom-input-group">
                            <label class="form-label">{{ __('menumaster::message.live_preview') }}</label>
                            <div class="border rounded p-3 bg-light">
                                <div id="menuPreview" class="d-flex align-items-center">
                                    ...
                                    <span id="previewModule" class="badge bg-secondary ms-2" style="display: none;"></span>
                        ...
                        @if ($menuMaster->children && $menuMaster->children->count() > 0)
                            <div class="col-12 col-sm-12 col-md-12 col-lg-12 form-group custom-input-group">
                                <label class="form-label">{{ __('menumaster::message.child_menus') }} ({{ $menuMaster->children->count() }})</label>
                                <div class="alert alert-warning">
                                    <i class="fas fa-exclamation-triangle"></i>
                                    <strong>{{ __('menumaster::message.note') }}:</strong>
                                    {{ __('menumaster::message.children_warning') }}
                                </div>
                                <div class="list-group">
                                    @foreach ($menuMaster->children as $child)
                                        <div class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <a href="{{ route('menumasters.edit', $child) }}" class="btn btn-sm btn-outline-primary">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 mt-1">
                            ...
                            <button type="button" class="btn btn-success float-start ms-2" onclick="duplicateMenu()">
                                <i class="fas fa-copy"></i> {{ __('menumaster::message.duplicate') }}
                            </button>
                            <button type="button" class="btn btn-danger float-end ms-2" onclick="deleteMenu()">
                                <i class="fas fa-trash"></i> {{ __('menumaster::message.delete') }}
                            </button>
                            <button type="submit" id="save" class="btn btn-primary float-end save">
                                <i class="fas fa-save"></i> {{ __('menumaster::message.update_menu_item') }}
                            </button>
                        </div>
                    </div>
                </form>
...
<script type="application/javascript">
    ...
    function duplicateMenu() {
        if (confirm('{{ __('menumaster::message.confirm_duplicate') }}')) {
    ...
    function deleteMenu() {
        ...
        if (confirm(confirmMessage)) {
            $.ajax({
                ...
                .fail(function() {
                    toastr.error('{{ __('menumaster::message.error_deleting') }}');
                });
        }
    }
    ...
    $(document).on('click', '.save', function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('menumasters.update', $menuMaster) }}",
            data: formData,
            dataType: 'json',
            cache: false,
            contentType: false,
            ...
            success: function(response) {
                $("#save").html('<i class="fas fa-save"></i> {{ __('menumaster::message.update_menu_item') }}');
                ...
                if (response.status_code == 500) {
                    ...
                    toastr.warning("Please fix the validation errors.", "Warning");
                } else if (response.status_code == 403) {
                    toastr.warning("Please input proper data.", "Warning");
                } else {
                    setTimeout(function() {
                        location.href = "{{ route('menumasters.index') }}";
                    }, 500);
