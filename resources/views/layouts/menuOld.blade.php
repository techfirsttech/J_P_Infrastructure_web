@php
    ...
    $hasPermissionForAnyChild = function ($menu) use (&$hasPermissionForAnyChild, $isMenuEnabled) {
        ...
        foreach ($menu->children as $child) {
            ...
            if ($childEnabled) {
                ...
                if (empty($child->if_can) || auth()->user()->can($child->if_can)) {
                    return true;
                }
    ...
    $hasPermissionForAnyGrandchild = function ($child) use ($isMenuEnabled) {
        ...
        foreach ($child->children as $grandchild) {
            ...
            if ($grandchildEnabled && (empty($grandchild->if_can) || auth()->user()->can($grandchild->if_can))) {
                return true;
            }
        }
...
<ul class="menu-inner pb-2 pb-xl-0">
    ...
    @foreach ($menus as $menu)
        ...
        @php
            ...
            $hasOwnPermission = empty($menu->if_can) || auth()->user()->can($menu->if_can);
                    ...
                    <ul class="menu-sub">
                        @foreach ($menu->children as $child)
                            @php
                                ...
                                $hasChildOwnPermission = empty($child->if_can) || auth()->user()->can($child->if_can);
